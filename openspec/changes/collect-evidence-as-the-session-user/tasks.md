...
- [x] 2.1 Rewrite `check_firewall_status()` to the evaluation table in `design.md`: the result line
...
- [x] 2.2 Assert that evaluation runs no firewall command: stub `sudo`, `iptables` and `nft` to fail
      loudly and verify `check-output` on a fixture never invokes them
- [x] 2.3 Re-run the regression against every tarball in the repository; verify 14-07 and 25-08 move
...
- [x] 3.1 Run the `gsettings` calls in the screen lock collection through
...
- [x] 3.2 Write `Settings read for user <name>` in the GNOME section, and `read as root` when no
...
- [x] 4.1 Add `firewall-info.txt` to the output file list in `README.md` and
      `ISO27001-LAPTOP-COMPLIANCE.md`, and name it in the firewall row of the compliance table
- [x] 4.2 Add a CHANGELOG entry under `## NEXT VERSION` covering the new evidence file, the removal
