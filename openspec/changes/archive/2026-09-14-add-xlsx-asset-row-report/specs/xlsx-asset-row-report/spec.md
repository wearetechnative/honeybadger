# xlsx-asset-row-report Capability
...
substituting an undetermined value for the affected columns rather than failing.

#### Scenario: A source file is missing
- **WHEN** one of the audit source files is absent from the output directory
- **THEN** the columns derived from that file report an undetermined value
