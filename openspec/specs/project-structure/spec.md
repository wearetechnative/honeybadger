...
## Purpose
Keep helper scripts and the shared library in a dedicated `lib/` directory while `RUNME.sh` stays the single, root-level entry point, so the repository layout is predictable and the user-facing workflow is unchanged.
