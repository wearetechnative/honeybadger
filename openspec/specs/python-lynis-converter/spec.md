# Python Lynis Converter

## Purpose
Convert a Lynis `.dat` report into JSON. The bundled Python script is the default path and needs no container; a Docker image running the Perl converter stays available behind `USE_DOCKER_CONVERTER=true`, pinned and rebuilt when its Dockerfile changes, and produces equivalent JSON.
