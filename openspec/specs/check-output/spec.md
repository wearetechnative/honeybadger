...
## Purpose
Let `./RUNME.sh check-output` read an audit bundle in either archive format — uncompressed `.tar` or gzip-compressed `.tar.gz` — picking the right extraction method automatically and producing the same report filename either way.
