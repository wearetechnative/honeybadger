...
## Purpose
This specification defines how the Honeybadger client submits audit reports to the collection server: which reports it sends, what it calls them, and how it reads the server's answer. Two paths exist - per-report submission (`submit`) and archive submission (`submit-tar`) - and they must agree on which reports exist and what they are called, because the server derives the report type from a header in one case and from the file name inside the archive in the other.
