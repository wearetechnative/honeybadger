...
## Purpose
This specification defines how the Honeybadger audit system collects and stores system information from neofetch. The system uses JSON format for structured data access, enabling reliable parsing with jq instead of brittle grep/sed text processing. Backward compatibility is maintained for existing audit archives that contain neofetch.txt files.
## Requirements
