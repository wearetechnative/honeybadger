# audit-identity-resolution Specification

## Purpose

Determines who ran an audit and when, for the reports the audit writes and for
`asset-inventory.json`. Both used to be recovered by parsing the output
directory's name, which joins hostname, username and date with `-` and lets all
three contain `-`; a hyphenated hostname shifted the fields and named a user
that does not exist on the machine. This capability names where each value
comes from, in what order, and what happens when none of the sources answer.
